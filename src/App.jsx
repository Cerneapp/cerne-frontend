...
export default function CerneApp() {
  ...
  async function startCamera(mode, wantsAudio) {
    ...
    try {
      ...
      setCameraError(
        "Não conseguimos acessar sua câmera. Escolha um arquivo da galeria abaixo.",
      );
      setCameraOn(false);
    }
  }
  ...
  function startRecording() {
    ...
    const supportedType = ["video/mp4", "video/webm"].find(
    ...
    if (story.authorId === userId) {
      try {
        const data = await apiFetch(
          `/stories/${story.id}/viewers?authorId=${userId}`,
          {},
          token,
        );
      ...
      try {
        await apiFetch(
          `/stories/${story.id}/view`,
          { method: "POST", body: JSON.stringify({ viewerId: userId }) },
          token,
        );
        setStories((prev) =>
  ...
  async function deleteStory(storyId) {
    try {
      await apiFetch(
        `/stories/${storyId}?authorId=${userId}`,
        { method: "DELETE" },
        token,
      );
  ...
  async function openHideList() {
    ...
    try {
      const list = await apiFetch(
        `/stories/hide-list?hiderId=${userId}`,
        {},
        token,
      );
  ...
  async function toggleHidePerson(personId) {
    ...
    try {
      await apiFetch(
        ...
        {
          method: "POST",
          body: JSON.stringify({ hiderId: userId, hiddenUserId: personId }),
        },
        token,
      );
      setHiddenUserIds((prev) =>
        isHidden ? prev.filter((id) => id !== personId) : [...prev, personId],
      );
    } catch (err) {
        ...
        {authMode === "reset" ? (
          resetDone ? (
            ...
            <form onSubmit={handleResetSubmit}>
              <p className="text-sm text-gray-600 mb-3">
                Crie uma senha nova pra sua conta.
              </p>
              <input
                type="password"
                value={newPassword}
          ...
          forgotSent ? (
            <div className="text-center">
              ...
              <p className="text-xs text-gray-500 mb-5">
                Se esse e-mail existir, um link de recuperação foi enviado.
            ...
            <form onSubmit={handleForgotSubmit}>
              ...
              <button
                type="submit"
                disabled={authLoading}
                className="w-full bg-blue-50 border border-blue-300 text-blue-700 rounded-lg py-3 text-sm font-medium disabled:opacity-60 mb-2"
              >
            ...
            <form onSubmit={handleAuthSubmit}>
              ...
              {authMode === "login" && (
                <p
                  ...
                  className="text-xs text-blue-600 text-right mb-4 cursor-pointer"
                >
                  Esqueceu a senha?
                </p>
              )}
      ...
      <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex gap-3 overflow-x-auto pb-1 -mt-1">
              {stories.find((s) => s.authorId === userId) ? (
                <button
                  onClick={() =>
                    openStory(stories.find((s) => s.authorId === userId))
                  }
                  className="text-center flex-shrink-0"
                >
                  <div className="w-14 h-14 rounded-full border-2 border-rose-400 flex items-center justify-center">
                    <div className="w-12 h-12 rounded-full bg-rose-100 text-rose-700 flex items-center justify-center text-xs font-medium">
                      EU
                    </div>
                  </div>
              ...
              {stories
                ...
                .map((s) => (
                  ...
                  >
                    <div
                      className={`w-14 h-14 rounded-full border-2 flex items-center justify-center ${s.viewedByMe ? "border-gray-200" : "border-rose-400"}`}
                    >
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <button
                      ...
                      onClick={() => openProfile(pulse.authorId)}
                    >
                  ...
                  {!pulse.own && !pulse.reacted && (
                    <div
                      className={
                        pulse.comments.length === 0
                          ? "border-t border-gray-100 pt-2 mb-1.5"
                          : "mb-1.5"
                      }
                    >
                  ...
                  <div
                    className={
                      pulse.own && pulse.comments.length === 0
                    ...
                    {openCommentId === pulse.id ? (
                      <div className="flex gap-2">
                        <input
                          ...
                          onKeyDown={(e) =>
                            e.key === "Enter" && submitComment(pulse)
                          }
                          className="flex-1 border border-gray-200 rounded-lg px-2 py-1.5 text-xs"
                        />
                        <button
                          onClick={() => submitComment(pulse)}
                          className="bg-blue-50 border border-blue-300 text-blue-700 rounded-lg px-3 text-xs font-medium"
                        >
                          enviar
                        </button>
        ...
        {tab === "explore" && (
          <div>
            ...
            <p className="text-xs text-gray-400 mb-2">
              comunidades com mais pulses
            </p>
            {interests.length === 0 && (
              <p className="text-xs text-gray-400 text-center py-8">
                Ainda não tem pulses com tags suficientes pra formar uma
                comunidade.
              </p>
            )}
            <div className="flex flex-col gap-2">
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400">
                        {c.pulseCount} {c.pulseCount === 1 ? "pulse" : "pulses"}
                      </p>
                    </div>
                  </button>
                );
        ...
        {tab === "profile" && (
          <div>
            ...
            <button
              onClick={saveProfile}
              className="w-full bg-blue-50 border border-blue-300 text-blue-700 rounded-lg py-3 text-sm font-medium mb-5"
            >
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {pulses
                ...
                .map((p) => (
                  <button
                    key={p.id}
                    onClick={() => setViewingOwnPulse(p)}
                    className="relative h-24 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center"
                  >
                    {p.mediaUrl ? (
                      p.mediaType === "video" ? (
                        <video
                      ...
                      <p className="text-[11px] text-gray-400 p-2 line-clamp-3 text-left">
                        {p.text}
                      </p>
                    )}
                    {p.mediaType === "video" && (
              ...
              {pulses.filter(
                (p) =>
                  p.own &&
                  (profileGridTab === "reels" ? p.mediaType === "video" : true),
              ).length === 0 && (
        ...
        <div className="absolute inset-0 bg-black flex flex-col">
          ...
          <div className="flex-1 relative bg-gray-900 flex items-center justify-center overflow-hidden">
            {createImagePreview ? (
              ...
              <p className="text-sm text-gray-300 px-8 text-center">
                {cameraError || "Carregando câmera..."}
              </p>
            )}
          ...
          {!createImagePreview ? (
            <div className="p-5 flex items-center justify-between">
              <label className="cursor-pointer">
                ...
                <input
                  type="file"
                  accept={
                    creatorMode === "reel" ? "video/*" : "image/*,video/*"
                  }
            ...
            <div className="bg-white p-4">
              ...
              <div className="flex gap-2">
                <button
                  onClick={retake}
                  className="flex-1 border border-gray-200 text-gray-500 rounded-lg py-2.5 text-sm font-medium"
                >
                  Refazer
                </button>
                <button
                  ...
                  disabled={
                    uploadingPhoto ||
                    publishing ||
                    (creatorMode !== "momento" && !createText.trim())
                  }
            ...
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-rose-100 text-rose-700 flex items-center justify-center text-xs font-medium">
                {viewingStory.authorId === userId
                  ? "EU"
                  : viewingStory.authorName.slice(0, 2).toUpperCase()}
              </div>
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  className="flex items-center gap-3 py-3 text-sm text-rose-600 w-full text-left border-b border-gray-100"
                >
                  <Trash2 className="w-[18px] h-[18px]" /> Apagar momento
                </button>
                <button
                  onClick={openHideList}
                  className="flex items-center gap-3 py-3 text-sm w-full text-left"
                >
                  <Eye className="w-[18px] h-[18px] text-gray-600" /> Ocultar de
                  alguém
                </button>
                <button
                  onClick={() => setStoryMenuOpen(false)}
                  className="w-full border border-gray-200 text-gray-500 rounded-lg py-2.5 text-sm font-medium mt-3"
                >
                  Cancelar
                </button>
              </div>
      ...
      {hideListOpen && (
        <div className="absolute inset-0 bg-white flex flex-col p-5">
          <div className="flex items-center justify-between mb-4">
            <ChevronLeft
              className="w-5 h-5 text-gray-500 cursor-pointer"
              onClick={() => setHideListOpen(false)}
            />
            <span className="text-sm font-medium">Ocultar momentos de</span>
            <span className="w-5" />
          </div>
          <p className="text-xs text-gray-400 mb-4">
            Quem você marcar aqui não vê mais nenhum dos seus momentos futuros,
            até você desmarcar.
          </p>
          {conversations.length === 0 && (
            <p className="text-xs text-gray-400 text-center py-8">
              Você ainda não tem matches pra ocultar.
            </p>
          )}
          <div className="flex flex-col gap-1">
            {conversations.map((c) => {
      ...
      {viewingOwnPulse && (
        <div className="absolute inset-0 bg-white flex flex-col">
          <div className="flex items-center justify-between p-4 border-b border-gray-100">
            <ChevronLeft
              className="w-5 h-5 text-gray-500 cursor-pointer"
              onClick={() => setViewingOwnPulse(null)}
            />
            ...
            {viewingOwnPulse.authorId === userId ? (
              <Trash2
                className="w-5 h-5 text-gray-400 cursor-pointer"
                onClick={() => {
                  removeOwnPulse(viewingOwnPulse.id);
                  setViewingOwnPulse(null);
                }}
              />
            ) : (
              <span className="w-5" />
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="flex items-center gap-2 mb-3">
              <Avatar
                initials={viewingOwnPulse.author.slice(0, 2).toUpperCase()}
                intentKey={viewingOwnPulse.intentKey}
              />
              <div>
            ...
            {viewingOwnPulse.hasPhoto && (
              <div
                className="w-full bg-gray-100 rounded-lg overflow-hidden mb-3"
                style={{ maxHeight: 400 }}
              >
                {viewingOwnPulse.mediaType === "video" ? (
                  <video
                    src={viewingOwnPulse.mediaUrl}
                    controls
                    className="w-full h-full object-contain"
                  />
                ) : (
        ...
        <div className="absolute inset-0 bg-white flex flex-col">
          <div className="flex items-center gap-3 p-4 border-b border-gray-100">
            ...
            <span className="text-sm font-medium">
              {viewingProfile.loading ? "..." : viewingProfile.name}
            </span>
          </div>
          <div className="flex-1 overflow-y-auto p-4">
            {viewingProfileLoading ? (
              ...
              <>
                <div className="text-center mb-4">
                  <Avatar
                    initials={viewingProfile.name.slice(0, 2).toUpperCase()}
                    intentKey={viewingProfile.intent}
                    size="w-16 h-16 text-lg mx-auto mb-2"
                  />
                ...
                <div className="flex justify-around py-3 mb-4 border-t border-b border-gray-100">
                  ...
                  <div className="text-center">
                    <p className="text-base font-medium">
                      {viewingProfile.mappedPulses.length}
                    </p>
                    <p className="text-[11px] text-gray-400">pulses</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {viewingProfile.mappedPulses.map((p) => (
                    <button
                      key={p.id}
                      onClick={() => setViewingOwnPulse(p)}
                      className="h-24 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center"
                    >
                      {p.mediaUrl ? (
                        p.mediaType === "video" ? (
                          <video
